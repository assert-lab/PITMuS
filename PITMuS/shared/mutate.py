"""Source-level mutation engine for PITMuS.

Turns a PIT bytecode mutation (as reported in mutations.xml) back into the
equivalent edit on the original Java *source*: locate the mutated statement,
resolve which operator/operand instance PIT actually targeted, apply the edit.

This is the single implementation, shared by both front-ends:

    scripts/gen_dataset.py   PIT report -> dataset CSVs
    scripts/inject.py        PIT report -> mutant .java files

Front-end-specific concerns stay out of here: CSV writing in gen_dataset.py,
bytecode occurrence resolution and .java emission in inject.py.
"""
...
# Re-export everything, including the _private helpers the front-ends call.
__all__ = [
    "COND_BOUNDARY_OPCODES",
    "COND_OPCODES",
    "INVOKE_OPCODES",
